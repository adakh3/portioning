...
- **Any change to seed data** (new dishes, menus, categories, rules, cost data, surcharges, etc.) **must regenerate `backend/seed.json`** by running: `cd backend && python manage.py dumpdata dishes menus rules bookings.SiteSettings bookings.ProductLine bookings.EventTypeOption bookings.SourceOption bookings.ServiceStyleOption bookings.LeadStatusOption bookings.LostReasonOption staff.LaborRole staff.AllocationRule equipment.EquipmentItem --indent 2 -o seed.json`
