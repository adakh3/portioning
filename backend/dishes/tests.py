...
def make_category(org=None, **kwargs):
    if org is None:
        org = Organisation.objects.create(name='Cat Org', slug='cat-org', country='PK')
    defaults = {
        "name": "test_cat",
...
def make_dish(org=None, category=None, **kwargs):
    if category is None:
        category = make_category(org=org)
    defaults = {
        "name": "Test Dish",
        "category": category,
...
class TestDishSellingPriceAutoCalc(TestCase):
    ...
    def test_auto_calculated_on_save(self):
        dish = make_dish(org=self.org, cost_per_gram=Decimal("0.0300"))
        # 0.03 / 0.30 = 0.10
    ...
    def test_different_target_percentage(self):
        ...
        self.settings.save()
        dish = make_dish(org=self.org, cost_per_gram=Decimal("0.0500"))
        # 0.05 / 0.25 = 0.20
        self.assertAlmostEqual(float(dish.selling_price_per_gram), 0.20, places=4)

    def test_zero_cost_leaves_selling_price_null(self):
        dish = make_dish(org=self.org, cost_per_gram=Decimal("0.0000"))
